...
export class Update extends plugin {
    ...
    async runUpdate(isForce) {
        const _path = './plugins/starlight-qsign/'
        ...
        this.oldCommitId = await this.getcommitId('starlight-qsign')
        uping = true
        let ret = await this.execSync(command)
        uping = false

        if (ret.error) {
            logger.mark(`${this.e.logFnc} 更新失败：星点签名插件`)
            this.gitErr(ret.error, ret.stdout)
            return false
        }
        /** 获取插件提交的最新时间 */
        let time = await this.getTime('starlight-qsign')

        if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
            await this.reply(`星点签名插件已经是最新版本\n最后更新时间：${time}`)
        } else {
            await this.reply(`星点签名插件\n最后更新时间：${time}`)
            this.isUp = true
            /** 获取星点签名组件的更新日志 */
            let log = await this.getLog('starlight-qsign')
